...
程序运行后会显示一个交互窗口，其中包含红色球体、紫色圆锥和材质参数面板。`Material` 面板提供四个滑动条：
...
运行 `optional.py` 时还会显示 `Light Position` 面板，用于实时调整点光源位置：

- `X`：光源横向位置，范围 `-5.0 ~ 5.0`，默认值 `2.0`
- `Y`：光源竖直位置，范围 `-5.0 ~ 6.0`，默认值 `3.0`
- `Z`：光源前后位置，范围 `-2.0 ~ 8.0`，默认值 `4.0`
...
- 摄像机位置：`(0, 0, 5)`
- 点光源初始位置：`(2, 3, 4)`，在 `optional.py` 中可通过滑动条实时调整
- 光源颜色：`(1.0, 1.0, 1.0)`
- 背景颜色：深青色
- 红色球体：
  - 球心：`(-1.2, -0.2, 0)`
...
`main.py` 中使用该标准 Phong 公式作为必做部分实现。`optional.py` 中则把高光计算替换为 Blinn-Phong 模型，并允许通过滑动条改变光源位置，用于完成选做内容和观察光照方向变化。
...
为了避免暗影射线再次命中当前表面造成自遮挡，代码对暗影射线起点沿光照方向进行了一个很小的偏移。调节光源位置后，暗影射线的方向和长度会随之变化，因此阴影边界也会实时移动。
...
- `shade()`：在 `main.py` 中计算标准 Phong 光照，在 `optional.py` 中根据当前光源位置计算 Blinn-Phong 高光并处理阴影。
- `render()`：对每个像素发射射线，完成求交、着色和颜色裁剪；`optional.py` 会接收 `light_x`、`light_y`、`light_z` 作为实时光源位置参数。
- `main()`：创建 Taichi UI 窗口和滑动条，实现材质参数与光源位置的实时交互。
...
调节 `Light Position` 的 `X` 坐标时，光源会在场景左右方向移动。受光面、高光位置和投影方向会随之横向偏移；当光源移动到物体另一侧时，原本明亮的一侧可能变暗，阴影也会向相反方向移动。

调节 `Y` 坐标时，光源高度发生变化。`Y` 较大时，物体上方和朝上的表面获得更强照明，阴影通常更靠近物体底部；`Y` 降低后，侧面照明更明显，部分上表面可能转为弱光或阴影区域。

调节 `Z` 坐标时，光源会在摄像机前后方向移动。`Z` 较大时，光源更靠近摄像机一侧，面向观察者的区域更容易出现高光；`Z` 减小时，光源逐渐移动到物体后方，正面亮度下降，背光和硬阴影效果更加明显。
...
通过本实验可以直观观察 `Ka`、`Kd`、`Ks`、`Shininess` 和光源位置对最终渲染效果的影响，加深对局部光照模型、暗影射线和实时交互式渲染流程的理解。
